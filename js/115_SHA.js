...
在2021.03.10 时在chrome与115pc（v24）上，转存与提取测试皆正常
...
* 如果转存失败，请检查链接或者在chrome上进行尝试，115pc端偶尔抽风
...
(function () {
    ...
    async function ByCommonFotmat(folderCid, text,needToCreateParentFolder) {
        ...
        msg = "完成上传！成功 <b>{0}</b> ，失败 <b>{1}</b> ，如果有失败，已将失败sha1链接复制到剪贴板！如果转存失败，请检查sha1链接格式或者在 chrome 上尝试转存。115 pc新版，接口正确下也会偶尔抽风。".format(files.length - fails.length, fails.length);
