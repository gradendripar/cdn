// ==UserScript==
// @name         115转存助手ui优化版v2.3(2021.0822更新)
// @name:zh      115转存助手ui优化版v2.3(2021.0822更新)
// @author       Never4Ever
// @namespace    Fake115Upload@Never4Ever
// @version      143.2021.0822.1
...
(function () {
    ...
    const TIPS = {
        CurrentVersion: "143.2021.0822.1",
        VersionTips: "115转存助手ui优化版v2.3(2021.0822更新)",
        UpdateUrl: "https://gist.github.com/Nerver4Ever/953447c9ecd330ffc0861d4cbb839369",
        AutoCreateFolder: "sha1转存时，强制在保存处新建根目录",
        AutoCreateFolderDetails: "选择时:&#013;&#010;1.新建根目录名来自sha1转存文件名或者json中的根元素。&#013;&#010;2.如果没有,则按编号(1-10)生成。&#013;&#010;如果未选择或者最终无法提取到目录名，则将保存处作为转存根目录",
        Sha1FileInputDetails: "如果不能正确显示选择文件按钮，可能是与其他脚本或者插件冲突！！",
        ItemNameSeparator: "sha1转存时，强制在文件名中加入分隔符",
        ItemNameSeparatorDetails: "如果选择此项，每个汉字或者每个字母之间强制加入分隔符,特殊标识如❤,可能会在其他程序无法识别",
    };

    const WORKSETTINGS = {
        WorkingItemsNumber: 4,//同时执行任务数
        SleepLittleTime: 500,//短暂休眠,毫秒,暂时在转存中使用
        SleepMoreTime: 1500,//长时休眠,毫秒,暂时在提取中使用
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (response) {
                    ...
                    if (response.status === 200 && response.response.status === 2) {
                        ...
                        console.error(response.response.statusmsg);
                        var error = "或许sha1链接不匹配?";
                        if (response.response && response.response.statusmsg) error = response.response.statusmsg;
                        else if (response.response) error = response.response;
    ...
    async function ByCommonFotmat(folderCid, text, folderNameSeparator) {
        ...
        for (var line of lines) {
            ...
            if (r.state) {
                if (folderNameSeparator && folderNameSeparator != "") {
                    let nameStrings = r.fileItem.name.split(".");
                    let extension = nameStrings.pop();
                    let fileName = nameStrings.join().split("").map(c => c + folderNameSeparator).join("").slice(0, -1);

                    r.fileItem.name = fileName + "." + extension;
                }
        ...
        for (var file of files) {
            ...
            if (index % WORKSETTINGS.WorkingItemsNumber == 0) {
                await delay(WORKSETTINGS.SleepLittleTime);
            }
        ...
        msg = '完成上传！成功 <b>{0}</b> ，失败 <b>{1}</b> '.format(files.length - fails.length, fails.length)
            + '<br><br>如果有失败，已将失败sha1链接复制到剪贴板！如果转存失败，请检查sha1链接格式或者在 chrome 上尝试转存。'
            + '获取最新版，或者遇到问题去此反馈，感谢 !点击-> <a href="' + TIPS.UpdateUrl
            + '" target="_blank">' + TIPS.VersionTips + '</a>';
        postSha1Messgae(createMessage(MessageType.END4UPLOAD, msg, folderCid));
    }

    //通过sha1链接转存文件
    //uploadSetting:{targetCid,text,rootFolder:{needToCreate:true,folderName:""},itemNameSeparator:{needToSeparate:true,separator:""}}
    async function UploadFilesBySha1Links(uploadSetting) {
        console.log("转存参数：");
        console.log(uploadSetting);
        console.log("转存参数结束");
        let formatText = uploadSetting.text;

        if (!formatText) return;
        postSha1Messgae(createMessage(MessageType.BEGIN4UPLOAD, "正在解析sha1链接..."));

        if (formatText.startsWith('{') && formatText.endsWith('}')) {
            let r = formatJsonToCommon(formatText);
            if (r.state) {
                uploadSetting.rootFolder.folderName = r.rootFolder;
                formatText = r.text;
            }
        }

        let newTargetCiD = uploadSetting.targetCid;
        //创建根目录
        if (uploadSetting.rootFolder.needToCreate == true) {
            for (let i = 0; i < 11; i++) {
                let targetFolderName = uploadSetting.rootFolder.folderName;
                postSha1Messgae(createMessage(MessageType.PROCESSING, "正在自动创建根目录{0}...".format(targetFolderName)));
                if (i != 0) {
                    targetFolderName = targetFolderName + "_" + i;
                }
                let tr = await addFolder(newTargetCiD, targetFolderName);
                if (tr.state) {
                    newTargetCiD = tr.cid;
                    postSha1Messgae(createMessage(MessageType.PROCESSING, "自动创建根目录{0}成功！".format(targetFolderName)));

                    break;
                }
                else {
                    postSha1Messgae(createMessage(MessageType.PROCESSING, "自动创建根目录{0}失败！原因：{1}，将自动尝试新的名字...".format(targetFolderName, tr.error)));
                    await delay(500);
                }
            }
        }
        ...
        //设置分隔符
        let separator = "";
        if (uploadSetting.itemNameSeparator.needToSeparate == true) {
            separator = uploadSetting.itemNameSeparator.separator;
        }


        await ByCommonFotmat(newTargetCiD, formatText, separator);
    }
    ...
    async function InnerCreateSha1Links(files, txtName) {
        ...
        for (var file of files) {
            ...
            if (gt1200files) {
                if (index % WORKSETTINGS.WorkingItemsNumber == 0) {
                    await Promise.all(promisArray);
                    await delay(WORKSETTINGS.SleepMoreTime);
    ...
    const autoCreateFolderString = `<div class="linktask-quota" style="margin-top: 10px;display: block">` +
        `<a>` + TIPS.AutoCreateFolder + `</a>` +
        `<div class="help" title="` + TIPS.AutoCreateFolderDetails + `"><a></a></div>` +
        `<span>&nbsp;&nbsp;</span>` +
        `<div class="option-switch" style="top:10px;left:10px">` +
        `<input type="checkbox" checked="true" id="downsha1autocreatefolder" onclick="function f() {return false}">` +
        `<label for><i>开启</i><s>关闭</s><b>切换</b></label></div>`;

    const inputFileOfSha1FileString = `<div class="linktask-quota" style="margin-top: 10px;">` +
        `<a>或者导入sha1链接文件（txt/json）</a>` +
        `<div class="help" title="` + TIPS.Sha1FileInputDetails + `"><a></a></div>` +
        `<span>&nbsp;&nbsp;</span>` +
        `<input type="file" id="downsha1file" accept=".txt,.json"></input></div>`;

    const itemNameSeparatorString = `<div class="linktask-quota" style="margin-top: 10px;">` +
        `<a>` + TIPS.ItemNameSeparator + `</a>` +
        `<select id="currentNameSeparator" style="width:auto;height:30px;border:1px solid #266ee3">` +
        `<option value="_">_(下划线)</option>` +
        `<option value=".">.</option>` +
        `<option value=" ">(空格)</option>` +
        `<option value="❤">❤</option>` +
        `<option value="@">@</option></select>` +
        `<div class="help" title="` + TIPS.ItemNameSeparatorDetails + `"><a></a></div>` +
        `<span>&nbsp;&nbsp;</span>` +
        `<div class="option-switch" style="left:10px">` +
        `<input type="checkbox" checked="false" id="itemNameSeparator" onclick="function f() {return true}">` +
        `<label for><i>开启</i><s>关闭</s><b>切换</b></label></div>`;

    const headerString = `<div id="my115tipsforheader">`
        + TIPS.VersionTips + `,<a style="color:red;" target="_blank" href=` + TIPS.UpdateUrl + `>更新&反馈点击此处!</a>`
        + `</div>`;
    ...
    function AddDownloadSha1Btn(jNode) {
        ...
        if (document.getElementById('my115tipsforheader') == null) {
        ...
        if (document.getElementById('downsha1file') == null) {
            ...
            console.log(div);
            var $fileInput = $(inputFileOfSha1FileString);
            var $autoCreateFolder = $(autoCreateFolderString);
            var $itemNameSeparator = $(itemNameSeparatorString);
            div[0].style.display = 'grid';
            div[0].appendChild($fileInput[0]);
            div[0].appendChild($autoCreateFolder[0]);
            div[0].appendChild($itemNameSeparator[0]);

            document.getElementById('itemNameSeparator').checked = false;
            ...
            file = "";
            document.getElementById('downsha1autocreatefolder').checked = true;
        ...
        if (document.getElementById('downsha1') == null) {
            ...
            var $btn = $('<div class="con" id="downsha1"><a class="button" href="javascript:;">开始sha1转存</a></div>');
            ...
            $btn[0].addEventListener('click', e => {
                ...
                var needToCreateParentFolder = !document.getElementById('downsha1autocreatefolder').checked;
                ...
                let uploadSetting = {
                    ...
                    text: "",
                    rootFolder: {
                        needToCreate: document.getElementById('downsha1autocreatefolder').checked,
                        folderName: ""
                    },
                    itemNameSeparator:
                    {
                        needToSeparate: document.getElementById('itemNameSeparator').checked,
                        separator: document.getElementById('currentNameSeparator').value
                    }
                };
                ...
                if (file) {
                    ...
                    reader.addEventListener('load', function (t) {
                        uploadSetting.rootFolder.folderName = file.name.split(".").slice(0, -1).join();
                        uploadSetting.text = t.target.result;
                        UploadFilesBySha1Links(uploadSetting);
                        file = "";
                        //NewDownloadFileFromSha1Links(cid, text,needToCreateParentFolder);
                    });
                    reader.readAsText(file);
                }
                else {
                    var text = { FileName: "", Content: links };
                    uploadSetting.rootFolder.folderName = "";
                    uploadSetting.text = links;
                    UploadFilesBySha1Links(uploadSetting);
                    //NewDownloadFileFromSha1Links(cid, text,needToCreateParentFolder);
                }
